...
async function main() {
    ...
    // 3. Start Agents
    console.log("\n[3/7] Starting Agents...");
    const agentA = (0, child_process_1.spawn)("npm", ["run", "start:a"], {
        shell: true,
        stdio: "ignore",
        env: { ...process.env, SYNC_INTERVAL_MS: "5000" },
    });
    const agentB = (0, child_process_1.spawn)("npm", ["run", "start:b"], {
        shell: true,
        stdio: "ignore",
        env: { ...process.env, SYNC_INTERVAL_MS: "5000" },
    });
    ...
    try {
        run('nats kv put config test_key "value_from_site_b" --server localhost:5222');
    }
    catch (e) {
    ...
    try {
        const nc = await (0, nats_1.connect)({ servers: "nats://localhost:4222" });
